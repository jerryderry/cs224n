...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    predicted -- numpy ndarray, predicted word vector (\hat{v} in
                 the written component), 1 by d vector
    target -- integer, the index of the target word
    outputVectors -- "output" vectors (as rows) for all tokens, V by d matrix
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    predicted -- center word vector, 1 by d vector
    target -- index of the current context word vector in outputVectors, a scalar
    outputVectors -- word vectors when used as context words, V by d matrix

    Return:
        cost -- loss function value, a scalar
        gradPred -- gradient of loss function with respect to center word vector, 1 by d vector
        grad -- gradient of loss function with respect to all context word vectors, V by d matrix
    """
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    gradIn -- the gradient with respect to the word vectors [V]
    gradOut -- the gradient with respect to the word vectors [U]
    """

    cost = 0.0
    gradIn = np.zeros(inputVectors.shape)       # V by d
    gradOut = np.zeros(outputVectors.shape)     # V by d
