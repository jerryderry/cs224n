...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        unconcatenated_embeddings = tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder)
        embeddings = tf.reshape(unconcatenated_embeddings, (-1, self.config.n_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.Variable(tf.random_uniform((1, self.config.hidden_size)))
        U = tf.Variable(xavier_initializer((self.config.hidden_size, self.config.n_classes)))
        b2 = tf.Variable(tf.random_uniform((1, self.config.n_classes)))
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.labels_placeholder,
                                                                         logits=pred))
        ### END YOUR CODE
        return loss
